...
(
echo include %filename%/NewBlock%filename%.pug
)> newFile.pug
type NewBlock.pug >> newFile.pug
move /Y newFile.pug  NewBlock.pug
(
...
(
echo @import '%fileName%/NewBlock%fileName%';
)>>NewBlock.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=NewBlock
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=NewBlock%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/NewBlock%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
